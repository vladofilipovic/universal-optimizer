...
FunctionOneVariableProblemMaxElements = NamedTuple('FunctionOneVariableProblemMaxElements', 
            [('expression',str), 
            ('domain_low',float), 
            ('domain_high',float)]
        )

class FunctionOneVariableProblemMax(TargetProblem):
    ...
    def __init__(self, expression:str, domain_low:float, domain_high:float)->None:
        ...
        super().__init__(name="FunctionOneVariableProblemMax", is_minimization=False, is_multi_objective=False)
    ...
    def __load_from_file__(cls, file_path:str, data_format:str)->int:
        ...
        if data_format=='txt':
                ...
                if len(data)>=3:
                    return FunctionOneVariableProblemMaxElements(data[0], float(data[1]), float(data[2]))
                else:
    ...
    def from_input_file(cls, input_file_path:str, input_format:str):
        ...
        """
        params:FunctionOneVariableProblemMaxElements = FunctionOneVariableProblemMax.__load_from_file__(input_file_path, 
                input_format)
